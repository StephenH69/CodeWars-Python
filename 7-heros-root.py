# One of the first algorithm used for approximating the integer square
...
# an error e and repeatedly calculating a new approximate integer value x using:
# (x + n / x) / 2;
...
# parameter guess (positive integer) which will be our initial x. 
...
# The kata is to return the count of the progression of integer approximations
...
def int_rac(n, guess):
    ...
    resultList = []
    while diff >= 1:
        resultList.append(int(guess))
        newGuess = ((guess + n / guess)/2)
        diff = abs(int(guess) - int(newGuess))
        guess = int(newGuess)
    print(resultList)
    return len(resultList)
print(int_rac(26, 1))
print(int_rac(125348, 300))
print(int_rac(125348981764,356243))

# Test.assert_equals(int_rac(25, 1), 4)
# Test.assert_equals(int_rac(125348, 300), 3)